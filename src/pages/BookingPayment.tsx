...
const BookingPayment = () => {
  ...
  const { selectedProvider, selectedServiceId, selectedDate, selectedTime, bookingNotes, dispatch } = useApp();
  const [method, setMethod] = useState<"wallet" | "upi">("wallet");
  const [loading, setLoading] = useState(false);

  if (!selectedProvider || !selectedServiceId || !selectedDate || !selectedTime) {
    navigate("/home", { replace: true });
    return null;
  }

  const base = selectedProvider.pricePerHr;
